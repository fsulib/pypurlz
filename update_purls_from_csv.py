...
def exec(host, username, password, purl_csv):
    ...
    with open(purl_csv, 'r') as read_obj:
        ...
        for row in csv_dict_reader:
            ...
            output = subprocess.check_output("./update_single_purl.py --host {host} --username {username} --password {password} --domain {domain}  --id {id} --target {target}".format(host=host, username=username, password=password, domain=domain, id=id, target=target), shell=True)
